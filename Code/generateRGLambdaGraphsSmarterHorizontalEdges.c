...
int sumHorizontalEdgesPerGroup[MAXVERTICES];
int nbLeavesPerGroup;
int nbGroups;
bitset alreadyTriedBitset;
...
void recursivelyAddEdges(struct graph* g, int iteration)
{
    ...
    if(isEmpty(compl(verticesWithCorrectDegree,n)))
    {
        ...
        for(int i=0; i<n && shouldOutput; i++)
        {
            forEach(neigh,g->adjacencyList[i])
            {
                ...
                if(!(1+lengthPath==girthRequired && numPaths==lambda))
                {
                    shouldOutput=false;
                    break;
                }
    ...
    bool sumPerGroupOk=true;
    int firstGroupNotOk=0;
    for(int i=0; i<nbGroups; i++)
    {
        if(sumHorizontalEdgesPerGroup[i] != lambda)
        {
            sumPerGroupOk=false;
            firstGroupNotOk=i;
            break;
    ...
    int groupStart=startLeaf+firstGroupNotOk*nbLeavesPerGroup;
    int groupFinish=startLeaf+(firstGroupNotOk+1)*nbLeavesPerGroup-1;
    bitset oldAlreadyTriedBitset=alreadyTriedBitset;
    //fprintf(stderr,"Iteration: %d\n",iteration);
    //fprintf(stderr,"sumPerGroupOk: %d\n",sumPerGroupOk);
    bitset valChanged1=EMPTY; // for which vertices x did we change validNeighbors[x]
    while(true)
    {
        int leastNumberOptions=n+5;
        int argMinn=-1;
        // check which vertex has least number of options
        forEachAfterIndex(i,compl(verticesWithCorrectDegree,n),groupStart-1)
        {
            if(!sumPerGroupOk && i>groupFinish) break;
            /*if(!sumPerGroupOk)
            {
                fprintf(stderr,"sum not ok yet and now looking at number of options for %d\n",i);
                fprintf(stderr,"alreadyTriedBitset:\n");
                for(int j=0; j<n; j++)
                {
                    if(contains(alreadyTriedBitset,j)) fprintf(stderr," %d",j);
                }
                fprintf(stderr,"\n");
            }*/
            if(!sumPerGroupOk && contains(alreadyTriedBitset,i)) continue;
        ...
        if(argMinn==-1)
        {
            //fprintf(stderr,"returned because argMinn was -1\n");
            forEach(u,valChanged1)
            {
                validNeighbors[u]=oldValidNeighbors1[iteration][u];
            }
            alreadyTriedBitset=oldAlreadyTriedBitset;
            return;
        }
        ...
        forEach(neigh,setToIterateOver)
        {
            ...
            int group1=(argMinn-startLeaf)/nbLeavesPerGroup;
            int group2=(neigh-startLeaf)/nbLeavesPerGroup;
            if(sumPerGroupOk && group1<nbGroups && group2<nbGroups) continue; // attempting horizontal edge
            if(!sumPerGroupOk && (group1>=nbGroups || group2>=nbGroups)) continue; // attempting non-horizontal edge
            // would result in more than lambda girth-cycles for some edge
            if(!sumPerGroupOk && (sumHorizontalEdgesPerGroup[group1]>=lambda || sumHorizontalEdgesPerGroup[group2]>=lambda)) continue;
            //fprintf(stderr,"Checking %d and %d at iteration %d\n",argMinn,neigh,iteration);
            ...
            if(!sumPerGroupOk)
            {
                sumHorizontalEdgesPerGroup[group1]++;
                sumHorizontalEdgesPerGroup[group2]++;
            }        
            ...
            if(shouldRecurse)
            {
                int lengthPath;
                int numPaths;
                calcNumberShortestPaths(g,argMinn,neigh,&lengthPath,&numPaths);
                if(1+lengthPath==girthRequired && numPaths>lambda)
                {
                    shouldRecurse=false;   
                }
        ...
        add(alreadyTriedBitset,argMinn);
    }
}
...
int main(int argc, char ** argv) {
    ...
    if(girthRequired%2==1)
    {
        nbGroups=r;
        nbLeavesPerGroup=1;
        for(int i=0; i<(girthRequired-3)/2; i++) nbLeavesPerGroup*=(r-1);
    }
    else
    {
        nbGroups=2;
        nbLeavesPerGroup=1;
        for(int i=0; i<(girthRequired-2)/2; i++) nbLeavesPerGroup*=(r-1);
    }
    alreadyTriedBitset=EMPTY;
    for(int i=0; i<nbGroups; i++) sumHorizontalEdgesPerGroup[i]=0;
    buildMooreTree(&g);
